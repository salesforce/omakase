 ...
 * @author nmcwilliams
 */
public enum PropertyName implements Property {
    ...
    /** reverse lookup map */
    private static final Map<String, PropertyName> map;
    static {
        Builder<String, PropertyName> builder = ImmutableMap.<String, PropertyName>builder();
        for (PropertyName pn : PropertyName.values()) {
            builder.put(pn.propertyName(), pn);
        }
        map = builder.build();
    }
     ...
     * @return The associated {@link PropertyName}, or a new instance of an {@link UnknownProperty} if not found.
     */
    public static Property from(String name) {
        PropertyName matched = map.get(name.toLowerCase());
