 ...
 * operations will rely on. For example, the {@link SyntaxTree} uses it to link related objects together, and {@link
 * AutoRefiner} uses it to automatically refine certain detailed syntax units, ensuring that they are received by {@link
 * Rework} plugins.
 * <p/>
 * In most cases this is <b>not</b> what you want to use. Prefer {@link Rework} for operations that change content, {@link
 * Observe} for operations that do not change content, and {@link Validate} for validation that can result in throwing an
 * error (via an {@link ErrorManager}).
 *
 * @author nmcwilliams
 */
